...
@section('content')
    ...
    <div class="col-span-9 bg-gray-600 shadow rounded px-6 pt-5 pb-7 text-gray-200">
        ...
        @foreach($orders as $order)
            <div class="main-box border border-gray-200 rounded-xl pt-6 mt-5">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between px-6 pb-6 border-b border-gray-200 text-gray-200">
                    <div class="data">
                        <p class="font-semibold text-base leading-7 text-black">N° de commande: <span class="font-medium text-gray-200">#{{$order->id}}</span></p>
                        <p class="font-semibold text-base leading-7 text-black mt-4">Statut de la commande: <span class="font-medium">@if ($order->status == 'COMPLETED') <span class="text-green-400 font-medium">Commande validée</span> @else <span class="text-red-800">En attente de confirmation du vendeur @endif</span></p>
                        <p class="font-semibold text-base leading-7 text-black mt-4">Date de paiement : <span class="font-medium text-gray-200">{{$order->created_at}}</span></p>
                    </div>
                ...
                @foreach($order->items as $item)
                    <div class="w-full px-3 min-[400px]:px-6">
                        <div class="flex flex-col lg:flex-row items-center py-6 border-b border-gray-200 gap-6 w-full">
                            <div class="img-box">
                                <img src="{{ asset('storage/upload/images/products/' . $item->product->images->first()->path) }}" class="h-20 w-20 transition-all duration-300 rounded-lg">
                            </div>
                                    ...
                                    <div class="grid grid-cols-5">
                                        <div class="col-span-5 lg:col-span-1 flex items-center max-lg:mt-3">
                                            <div class="flex gap-3 lg:block">
                                                ...
                                                <p class="lg:mt-4 font-medium text-sm leading-7 text-gray-200">{{$item->product->price * $item->quantity}} $</p>
                                            </div>
                                        </div>
                ...
                @if ($amountDiscount > 0)
                    <div class="w-full px-6 flex flex-col lg:flex-row items-center justify-between">
                        ...
                        <p class="font-semibold text-lg text-black py-6">Montant de la réduction: <span class="text-green-600"> {{$amountDiscount}} $ </span></p>
                ...
                <div class="w-full border-t border-gray-200 px-6 flex flex-col lg:flex-row items-center justify-between ">
                    ...
                    <p class="font-semibold text-lg text-black py-6">@if ($amountDiscount > 0) Sous-total: <span class="text-indigo-600">{{$checkoutTotalAmount}} $</span><br> @endif Prix total: <span class="text-green-600"> {{$checkoutTotalAmount - $amountDiscount}} $</span></p>
                </div>
            </div>
        @endforeach
    </div>
