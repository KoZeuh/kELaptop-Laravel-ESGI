...
@section('content')
    <div id="default-carousel" class="relative rounded-lg overflow-hidden shadow-lg" data-carousel="static">
        ...
        <button type="button" class="flex absolute top-1/2 left-3 z-40 items-center justify-center w-10 h-10 bg-gray-200/50 rounded-full hover:bg-gray-300 focus:outline-none transition" data-carousel-prev>
            <svg class="w-5 h-5 text-gray" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="container text-gray-200">
        <h2 class="text-2xl font-medium uppercase mb-6">Nos catégories</h2>
        <div class="grid grid-cols-3 gap-3">
            @foreach ($categories as $category)
                @if ($category->parent_id == null)
                    <div class="relative rounded-sm overflow-hidden group">
                        <img src="{{ asset('storage/upload/images/categories/'. $category->path_banner)}}" class="h-full">
                        <a href="#" class="absolute inset-0 bg-black bg-opacity-40 flex justify-center text-xl font-roboto font-medium group-hover:bg-opacity-60 transition">{{ $category->name }}</a>
                    </div>
                @endif
            @endforeach
        </div>
    ...
    <div class="container py-16 text-gray-200">
        <h2 class="text-2xl font-medium text-center uppercase mb-6">Recommandé pour vous</h2>
